...
func (c *ChatGPTConnector) GetOrUpdateGhost(ctx context.Context, ghostID networkid.UserID, model string) (*bridgev2.Ghost, error) {
	...
	if ghost.Name == "" || needsUpdate {
		...
		displayName := fmt.Sprintf("ChatGPT (%s)", modelName)

		isBot := true
		userInfo := &bridgev2.UserInfo{
			Name:        &displayName,
			IsBot:       &isBot,
			Identifiers: []string{fmt.Sprintf("chatgpt:%s", modelName)},
		}
...
// GetUserInfo returns information about a ghost user (OpenAI model).
func (c *ChatGPTClient) GetUserInfo(ctx context.Context, ghost *bridgev2.Ghost) (*bridgev2.UserInfo, error) {
	meta, ok := ghost.Metadata.(*GhostMetadata)
	if !ok || meta == nil {
	...
	if info := chatgptapi.GetModelInfo(modelName); info != nil && info.DisplayName != "" {
