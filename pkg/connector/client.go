...
// isImageSupported checks if a MIME type is supported by OpenAI Vision.
...
func (c *ChatGPTClient) downloadAndEncodeImage(ctx context.Context, content *event.MessageEventContent) (*chatgptapi.Content, error) {
	// Get the content URI
	uri := content.URL
	...
	c.Connector.Log.Debug().
		...
		Msg("Downloaded and encoded image for OpenAI Vision")

	// OpenAI Vision uses data URI format for base64 images
	dataURI := fmt.Sprintf("data:%s;base64,%s", mimeType, base64Data)

	return &chatgptapi.Content{
		Type: "image_url",
		ImageURL: &chatgptapi.ImageURL{
			URL:    dataURI,
			Detail: "auto",
		},
	}, nil
...
// recentMention tracks when a user mentioned ChatGPT in a portal.
...
// ChatGPTClient represents a client connection to OpenAI ChatGPT API.
type ChatGPTClient struct {
	MessageClient chatgptapi.MessageClient // Can be *chatgptapi.Client or *chatgptapi.WebClient
	UserLogin     *bridgev2.UserLogin
	Connector     *ChatGPTConnector
	conversations map[networkid.PortalID]*chatgptapi.ConversationManager
	convMu        sync.RWMutex
...
var (
	_ bridgev2.NetworkAPI                    = (*ChatGPTClient)(nil)
	_ bridgev2.IdentifierResolvingNetworkAPI = (*ChatGPTClient)(nil)
	_ bridgev2.MembershipHandlingNetworkAPI  = (*ChatGPTClient)(nil)
)

// Connect is called when the client should connect.
func (c *ChatGPTClient) Connect(ctx context.Context) {
	...
	c.Connector.Log.Info().Msg("ChatGPT client ready")
...
func (c *ChatGPTClient) Disconnect() {
	...
	c.Connector.Log.Info().Msg("ChatGPT client disconnected")
}

// conversationCleanupLoop periodically cleans up expired conversations.
func (c *ChatGPTClient) conversationCleanupLoop() {
	defer c.wg.Done()
	defer func() {
		if r := recover(); r != nil {
...
func (c *ChatGPTClient) cleanupExpiredConversations(maxAge time.Duration) {
	c.convMu.Lock()
...
func (c *ChatGPTClient) IsLoggedIn() bool {
	return c.MessageClient != nil
}

// LogoutRemote logs out from the remote service.
// For API key logins: no remote cleanup needed (key remains valid until revoked at platform.openai.com).
func (c *ChatGPTClient) LogoutRemote(ctx context.Context) {
	// No remote cleanup needed for API key logins
}

// getAPIKey returns the API key from the user login metadata.
func (c *ChatGPTClient) getAPIKey() string {
	if c.UserLogin == nil {
		return ""
	}
...
func (c *ChatGPTClient) IsThisUser(ctx context.Context, userID networkid.UserID) bool {
	// All ChatGPT ghosts belong to the system, not individual users
	return false
}

// isChatGPTMentioned checks if the ChatGPT ghost is mentioned in the message.
func (c *ChatGPTClient) isChatGPTMentioned(msg *bridgev2.MatrixMessage) bool {
	// Check formatted body for mentions (HTML format)
	if msg.Content.FormattedBody != "" {
		// Look for mention pill: <a href="https://matrix.to/#/@chatgpt_
		if strings.Contains(msg.Content.FormattedBody, "/@chatgpt_") {
			return true
		}
		...
		ghostID := c.Connector.MakeChatGPTGhostID(model)
		ghostMXID := fmt.Sprintf("@chatgpt_%s:", ghostID)
		if strings.Contains(msg.Content.FormattedBody, ghostMXID) {
			return true
		}
	}

	// Check plain text body for @chatgpt mentions
...
// Used to detect messages like "@chatgpt_gpt4o:server.com" that are just triggering image processing.
func (c *ChatGPTClient) isMentionOnlyMessage(msg *bridgev2.MatrixMessage) bool {
	// Get the plain text body
	body := strings.TrimSpace(msg.Content.Body)

	// Remove Matrix MXIDs that start with @chatgpt (e.g., @chatgpt_gpt4o:server.com)
	// These are the actual mention format in Matrix
	cleaned := body
	for {
		atIdx := strings.Index(strings.ToLower(cleaned), "@chatgpt")
		if atIdx == -1 {
			break
		}
	...
	// Remove standalone "ChatGPT" (display name mention)
	cleaned = strings.ReplaceAll(cleaned, "ChatGPT", "")
	cleaned = strings.ReplaceAll(cleaned, "chatgpt", "")
...
// recordMention records that a user mentioned ChatGPT in a portal.
// This allows subsequent images from the same user to be processed.
func (c *ChatGPTClient) recordMention(userID id.UserID, portalID networkid.PortalID) {
	c.mentionMu.Lock()
...
// consumeRecentMention checks if a user has recently mentioned ChatGPT in a portal.
...
func (c *ChatGPTClient) consumeRecentMention(userID id.UserID, portalID networkid.PortalID) bool {
	c.mentionMu.Lock()
	defer c.mentionMu.Unlock()
...
func (c *ChatGPTClient) getConversationManager(portal *bridgev2.Portal) *chatgptapi.ConversationManager {
	c.convMu.Lock()
	defer c.convMu.Unlock()
	...
	maxTokens := chatgptapi.GetModelMaxTokens(c.Connector.Config.GetDefaultModel())
	cm := chatgptapi.NewConversationManager(maxTokens)
	c.conversations[portalID] = cm

	return cm
}

// HandleMatrixMessage handles a message sent from Matrix to ChatGPT.
func (c *ChatGPTClient) HandleMatrixMessage(ctx context.Context, msg *bridgev2.MatrixMessage) (*bridgev2.MatrixMessageResponse, error) {
	...
	if meta.MentionOnly {
		mentioned := c.isChatGPTMentioned(msg)
		isImage := msg.Content.MsgType == event.MsgImage

		if mentioned {
			// Check if this is a mention-only message (e.g., just "@chatgpt" with no real content)
			// These are typically sent as captions for images, so don't send to ChatGPT - wait for the image
			...
			c.Connector.Log.Debug().Msg("Mention-only mode: ChatGPT mentioned, processing message")
			...
			c.Connector.Log.Debug().Msg("Mention-only mode: ChatGPT not mentioned, ignoring message")
	...
	var messageContent []chatgptapi.Content
	...
	case event.MsgImage:
		...
		if msg.Content.Body != "" && msg.Content.Body != msg.Content.FileName {
			...
			messageContent = append(messageContent, chatgptapi.Content{
				Type: "text",
				Text: captionText,
			})
		} else {
		...
		c.Connector.Log.Info().
			...
			Msg("Processing image message with OpenAI Vision")
	...
	// Model ID is passed directly to OpenAI API - no resolution needed, API validates at runtime
	model := meta.Model
	if model == "" {
		model = c.Connector.Config.GetDefaultModel()
	}
	...
	// Build messages for API request: send existing history + new user message
	userMessage := chatgptapi.Message{
		Role:    "user",
		Content: messageContent,
	}

	var messagesForAPI []chatgptapi.Message

	{
		// Include local conversation history
		...
		if convMgr.NeedsCompaction() {
			...
			// Call ChatGPT to generate a summary (using the direct API client)
			if apiClient, ok := c.MessageClient.(*chatgptapi.Client); ok {
	...
	req := &chatgptapi.CreateMessageRequest{
		Model:       model,
	...
	if stream == nil {
		stopTyping()
		errMsg := "received nil stream from OpenAI API"
		c.sendErrorToRoom(ctx, msg.Portal, errMsg)
	...
	for event := range stream {
		...
		case "message_start":
			if event.Message != nil {
				chatgptMessageID = event.Message.ID
	...
	if streamCtx.Err() == context.DeadlineExceeded {
		errMsg := fmt.Sprintf("Request timed out after %s. The API may be overloaded or unresponsive.", streamTimeout)
		c.Connector.Log.Error().Dur("timeout", streamTimeout).Msg("OpenAI API request timed out")
		c.sendErrorToRoom(ctx, msg.Portal, errMsg)
		return nil, errors.New(errMsg)
	} else if streamCtx.Err() == context.Canceled {
		errMsg := "Request was cancelled"
		c.Connector.Log.Warn().Msg("OpenAI API request was cancelled")
		return nil, errors.New(errMsg)
	}
	...
	if chatgptMessageID == "" {
		chatgptMessageID = fmt.Sprintf("msg_%d", time.Now().UnixNano())
	}

	// Validate response content
	responseContent := responseText.String()
	if responseContent == "" {
		errMsg := "received empty response from ChatGPT"
		c.sendErrorToRoom(ctx, msg.Portal, errMsg)
		return nil, errors.New(errMsg)
	}
	...
	convMgr.AddMessageWithID("assistant", responseContent, chatgptMessageID)
	...
	go func() {
		...
		c.queueAssistantResponse(msg.Portal, responseContent, chatgptMessageID, inputTokens+outputTokens)
	}()

	// Return response metadata for the outgoing (user -> ChatGPT) message
	// Use a unique ID based on user's Matrix event ID to avoid collision with ChatGPT's response ID
	return &bridgev2.MatrixMessageResponse{
		DB: &database.Message{
			ID:        networkid.MessageID("user_" + userMsgID),
			Timestamp: time.Now(),
			Metadata: &MessageMetadata{
				ChatGPTMessageID: "user_" + userMsgID,
...
func (c *ChatGPTClient) formatUserFriendlyError(err error) error {
	if err == nil {
		return nil
	}
	...
	if strings.Contains(errStr, "authentication failed") || strings.Contains(errStr, "401") {
		return fmt.Errorf("authentication failed. Your API key may be invalid - please check your OpenAI API key")
	}

	// Check for specific error types
	if chatgptapi.IsRateLimitError(err) {
		retryAfter := chatgptapi.GetRetryAfter(err)
		if retryAfter > 0 {
	...
	if chatgptapi.IsOverloadedError(err) {
		return fmt.Errorf("OpenAI API is currently overloaded. Please try again in a few moments")
	}

	if chatgptapi.IsInvalidRequestError(err) {
	...
	return fmt.Errorf("failed to send message to ChatGPT. Please try again later")
}

// sendCompactionNotice sends a notice to the Matrix room that context was compacted.
func (c *ChatGPTClient) sendCompactionNotice(ctx context.Context, portal *bridgev2.Portal) {
	...
	c.UserLogin.QueueRemoteEvent(&simplevent.Message[*MessageMetadata]{
		EventMeta: simplevent.EventMeta{
			...
			Sender:    bridgev2.EventSender{Sender: c.Connector.MakeChatGPTGhostID("system")},
			Timestamp: time.Now(),
		},
		ID: networkid.MessageID(fmt.Sprintf("compaction_%d", time.Now().UnixNano())),
		Data: &MessageMetadata{
			ChatGPTMessageID: "compaction",
		},
...
func (c *ChatGPTClient) sendErrorToRoom(ctx context.Context, portal *bridgev2.Portal, errorMsg string) {
	if ctx == nil || ctx.Err() != nil {
		return
	}
	...
	c.UserLogin.QueueRemoteEvent(&simplevent.Message[*MessageMetadata]{
		EventMeta: simplevent.EventMeta{
			...
			PortalKey: portal.PortalKey,
			Sender:    bridgev2.EventSender{Sender: c.Connector.MakeChatGPTGhostID("error")},
			Timestamp: time.Now(),
		},
		ID: networkid.MessageID(fmt.Sprintf("error_%d", time.Now().UnixNano())),
		Data: &MessageMetadata{
			ChatGPTMessageID: "error",
		},
		ConvertMessageFunc: func(ctx context.Context, portal *bridgev2.Portal, intent bridgev2.MatrixAPI, data *MessageMetadata) (*bridgev2.ConvertedMessage, error) {
			return &bridgev2.ConvertedMessage{
				Parts: []*bridgev2.ConvertedMessagePart{
...
func (c *ChatGPTClient) queueAssistantResponse(portal *bridgev2.Portal, text, messageID string, tokensUsed int) {
	model := c.Connector.Config.GetDefaultModel()
...
func (c *ChatGPTClient) sendMessageWithRetry(ctx context.Context, portal *bridgev2.Portal, ghost *bridgev2.Ghost, text, messageID string, tokensUsed int, maxSize int) {
	...
	if len(parts) > 1 {
		c.Connector.Log.Info().
			...
			Int("max_size", maxSize).
			Msg("Split large ChatGPT response into multiple messages")
	}
}
...
func (c *ChatGPTClient) sendSizeErrorNotice(ctx context.Context, portal *bridgev2.Portal, ghost *bridgev2.Ghost) {
	notice := "⚠️ Part of ChatGPT's response could not be delivered due to Matrix size limits."
	content := &event.MessageEventContent{
		MsgType: event.MsgNotice,
		Body:    notice,
	}
...
func (c *ChatGPTClient) GetCapabilities(ctx context.Context, portal *bridgev2.Portal) *event.RoomFeatures {
	return &event.RoomFeatures{
		...
		File: event.FileFeatureMap{
			// OpenAI Vision supports these image types
			event.MsgImage: {
				MaxSize: 20 * 1024 * 1024, // 20MB max for OpenAI Vision
		...
		MaxTextLength:       100000, // OpenAI models have large context windows
		...
		ReadReceipts:        false,
		TypingNotifications: true, // ChatGPT shows "typing" while processing
	}
}
...
func (c *ChatGPTClient) HandleMatrixEdit(ctx context.Context, msg *bridgev2.MatrixEdit) error {
...
func (c *ChatGPTClient) HandleMatrixMessageRemove(ctx context.Context, msg *bridgev2.MatrixMessageRemove) error {
	// Get the conversation manager for this portal
	convMgr := c.getConversationManager(msg.Portal)
...
func (c *ChatGPTClient) HandleMatrixReaction(ctx context.Context, msg *bridgev2.MatrixReaction) error {
	return fmt.Errorf("reactions are not supported")
}

// HandleMatrixReactionRemove handles removal of a reaction (not supported).
func (c *ChatGPTClient) HandleMatrixReactionRemove(ctx context.Context, msg *bridgev2.MatrixReactionRemove) error {
	return fmt.Errorf("reactions are not supported")
}

// HandleMatrixReadReceipt handles a read receipt (not supported).
func (c *ChatGPTClient) HandleMatrixReadReceipt(ctx context.Context, msg *bridgev2.MatrixReadReceipt) error {
	// Silently ignore read receipts
	return nil
}

// HandleMatrixTyping handles a typing notification (not supported).
func (c *ChatGPTClient) HandleMatrixTyping(ctx context.Context, msg *bridgev2.MatrixTyping) error {
	// Silently ignore typing notifications
	return nil
}

// HandleMatrixMembership handles membership changes including ghost invitations.
// This allows users to invite ChatGPT ghost users to rooms directly.
func (c *ChatGPTClient) HandleMatrixMembership(ctx context.Context, msg *bridgev2.MatrixMembershipChange) (*bridgev2.MatrixMembershipResult, error) {
	// We only care about invites to ghost users
	if msg.Type != bridgev2.Invite {
		return nil, nil
	}

	// Check if the target is a ghost (ChatGPT bot being invited)
	...
	c.Connector.Log.Info().
		...
		Msg("ChatGPT ghost invited to room, accepting invitation")
...
func (c *ChatGPTClient) PreHandleMatrixMessage(ctx context.Context, msg *bridgev2.MatrixMessage) (bridgev2.MatrixMessageResponse, error) {
	// No pre-processing needed
	return bridgev2.MatrixMessageResponse{}, nil
}

// GetMetrics returns the API client metrics.
func (c *ChatGPTClient) GetMetrics() *chatgptapi.Metrics {
...
func (c *ChatGPTClient) ClearConversation(portalID networkid.PortalID) {
	c.convMu.Lock()
	defer c.convMu.Unlock()
...
func (c *ChatGPTClient) GetConversationStats(portalID networkid.PortalID) (messageCount, estimatedTokens int, lastUsed time.Time) {
	c.convMu.RLock()
...
func (c *ChatGPTClient) GetConversationFullStats(portalID networkid.PortalID) (messageCount, estimatedTokens, maxTokens, compactionCount int, isCompacted bool) {
	c.convMu.RLock()
	defer c.convMu.RUnlock()
...
// The identifier is passed directly to the OpenAI API as the model ID.
func (c *ChatGPTClient) ResolveIdentifier(ctx context.Context, identifier string, createChat bool) (*bridgev2.ResolveIdentifierResponse, error) {
	c.Connector.Log.Debug().
	...
	if model == "" {
		return nil, fmt.Errorf("empty identifier provided - specify a model ID (e.g., 'gpt-4o')")
	}

	ghostID := c.Connector.MakeChatGPTGhostID(model)

	// Get display name for the model
	displayName := fmt.Sprintf("ChatGPT (%s)", model)
	if info := chatgptapi.GetModelInfo(model); info != nil && info.DisplayName != "" {
	...
	ghostUserInfo := &bridgev2.UserInfo{
		Name:        &displayName,
		IsBot:       &isBot,
		Identifiers: []string{fmt.Sprintf("chatgpt:%s", model)},
	}
	...
	conversationID := fmt.Sprintf("conv_%s_%d", chatgptapi.GetModelFamily(model), time.Now().UnixNano())
			...
			PortalInfo: &bridgev2.ChatInfo{
				...
				Members: &bridgev2.ChatMemberList{
					...
					Members: []bridgev2.ChatMember{
						...
						{
							// The ChatGPT ghost - include UserInfo for proper setup
							EventSender: bridgev2.EventSender{
								IsFromMe: false,
								Sender:   ghostID,
							},
...
// Model IDs are passed directly to the OpenAI API for validation.
func (c *ChatGPTClient) parseModelIdentifier(identifier string) string {
	identifier = strings.TrimSpace(identifier)
	if identifier == "" {
		return ""
	}

	// Check if it's a ghost ID format (e.g., "chatgpt_gpt_4o")
	// MakeChatGPTGhostID creates ghost IDs by replacing hyphens/dots with underscores
	// So we reverse that transformation here
	if strings.HasPrefix(strings.ToLower(identifier), "chatgpt_") {
		modelPart := identifier[8:] // len("chatgpt_") == 8
		return strings.ReplaceAll(modelPart, "_", "-")
	}

	// Pass the identifier directly to the API as the model ID
	// OpenAI will validate it at runtime
	return identifier
}
